...
function metricMarkup(label, value, unit = "", options = {}) {
  ...
  if (options.band && options.band.hi > 0) {
    ...
    band = `<div class="uncertainty ${wide ? "uncertainty--wide" : ""}"><div class="uncertainty__track"><i class="uncertainty__range" style="left:${pct(lo)}%;width:${Math.max(2, pct(hi) - pct(lo))}%"></i><b class="uncertainty__marker" style="left:${pct(est)}%"></b></div><div class="uncertainty__note"><span>${esc(fmtWatts(lo))} – ${esc(fmtWatts(hi))}</span><strong>${wide ? "wide" : "tight"}</strong></div></div>`;
  }
...
async function renderProfile(_, token = startPage()) {
  ...
  $("#save-profile").addEventListener("click", async () => {
    ...
    try { await api("/api/profile", { method: "PUT", body: { rider: { age: num("r-age"), weight_kg: num("r-weight"), height_cm: num("r-height"), resting_hr: num("r-rest"), max_hr: num("r-maxhr"), bike_type: $("#r-bike").value, hr_zones: hrZones }, bike: { id: bike.id, name: $("#b-name").value, mass_kg: num("b-mass"), crr: num("b-crr"), cdA: num("b-cda"), drivetrain_efficiency: num("b-eff"), calibrated: bike.calibrated } } }); toast("Profile saved and rides recalculated."); } catch (error) { toast(error.message); }
    saveButton.disabled = false; saveButton.textContent = saveLabel;
  });
  const calibrationList = $("#calibration-list");
  if (!calibrations.length) calibrationList.innerHTML = emptyState("No calibration yet", "The engine will record a suitable climb or closed loop here.");
  else calibrationList.innerHTML = `<div class="table-scroll"><table class="ride-table"><thead><tr><th>Ride</th><th>Type</th><th title="Rolling-resistance coefficient">Crr</th><th title="Drag area — coefficient of drag × frontal area">CdA</th><th title="Fit quality of the calibration">R²</th><th>Segments</th></tr></thead><tbody>${calibrations.map((calibration) => `<tr><td>${esc(calibration.filename || calibration.ride_id)}</td><td>${confidenceTag(calibration.type === "loop" ? "high" : "context", calibration.type)}</td><td class="mono">${calibration.params?.crr ? Number(calibration.params.crr).toFixed(4) : "—"}</td><td class="mono">${calibration.params?.cdA ? Number(calibration.params.cdA).toFixed(2) : "—"}</td><td class="mono">${calibration.r2 != null ? Number(calibration.r2).toFixed(2) : "—"}</td><td class="mono">${calibration.params?.n_segments || "—"}</td></tr>`).join("")}</tbody></table></div>`;
}
